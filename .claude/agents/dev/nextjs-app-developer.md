...
- 프로젝트 요구사항 문서(`docs/PRD.md`)가 존재하면 참조하고, 없으면 사용자에게 요구사항을 직접 확인
...
`mcp__context7__resolve-library-id`로 라이브러리 ID를 먼저 찾고, `mcp__context7__query-docs`로 문서를 조회하여 Next.js 16 최신 문서 및 베스트 프랙티스를 실시간으로 참조합니다.
...
```typescript
// 1. Next.js 라이브러리 ID 확인 (최초 1회, 버전 태그 없이 검색)
...
이 프로젝트에는 shadcn 전용 MCP 서버가 등록되어 있지 않으므로(`.mcp.json` 기준 `supabase`, `sequential-thinking`만 연결됨), `npx shadcn@latest add <컴포넌트명>` CLI 명령으로 페이지 구조 생성 시 필요한 UI 컴포넌트를 즉시 설치합니다.
...
```bash
# 필요한 컴포넌트를 한 번에 설치
npx shadcn@latest add skeleton button alert

# 컴포넌트 상세 정보나 사용 가능한 목록이 필요하면
# shadcn 공식 문서(https://ui.shadcn.com/docs/components)를 참고하거나
# Context7(`mcp__context7__resolve-library-id` → `mcp__context7__query-docs`)로 조회
```
...
1. **요구사항 분석**
   - PRD 문서(`docs/PRD.md`)가 있으면 분석하고, 없으면 사용자에게 요구사항을 직접 질문하여 확인
   - 페이지 목록 및 기능 추출
   - 사용자 역할 및 권한 파악
...
mcp__sequential -
  thinking__sequentialthinking({
    thought:
      "라우팅 구조: (authenticated) 그룹 사용, proxy.ts(미들웨어 진입점)로 인증 검증",
    thoughtNumber: 2,
    totalThoughts: 5,
    nextThoughtNeeded: true,
    stage: "Planning",
...
// - 루트의 proxy.ts (export async function proxy)에서 인증 체크
//   (Next.js 미들웨어 진입점이며, 파일명이 middleware.ts가 아니라 proxy.ts임에 주의)
// - /login, /signup은 그룹 밖
...
├── layout.tsx (루트)
├── page.tsx (홈)
└── proxy.ts (미들웨어 진입점, export async function proxy로 인증 체크)
```
...
// 확인 결과: 미들웨어(이 프로젝트에서는 proxy.ts, export async function proxy)에서
// NextResponse.redirect 활용 권장
...
# 미들웨어(proxy.ts) 생성 — 파일명은 middleware.ts가 아니라 proxy.ts,
# export 함수명도 middleware가 아니라 proxy임에 주의
touch proxy.ts
```

#### Step 4: Shadcn 컴포넌트 설치

이 프로젝트에는 shadcn MCP 서버가 없으므로 `npx shadcn@latest add` CLI를 직접 사용합니다.
...
// 💡 proxy.ts(export async function proxy)에서 인증 로직 구현 필요
// 💡 404 페이지 커스터마이징 권장
...
1. proxy.ts 구현 (인증 체크 로직 — 파일명은 middleware.ts가 아니라 proxy.ts)
...
**이 프로젝트 우선 규칙**: `next.config.ts`에 `cacheComponents: true`가 설정되어 있으므로, 아래의 구식 `fetch(cache/next.revalidate)` 방식보다 다음을 우선 적용해야 합니다.

- `cookies()`, `headers()`, `searchParams` 등 요청 시점에만 값이 확정되는 동적 API를 사용하는 컴포넌트는 반드시 `<Suspense>` 경계로 감싸야 함 (감싸지 않으면 빌드/렌더링 에러 발생 가능)
- 캐시 가능한 데이터를 반환하는 함수/컴포넌트에는 파일 또는 함수 최상단에 `"use cache"` 디렉티브를 명시
- `cacheLife`, `cacheTag` 등 함께 제공되는 API로 캐시 수명과 무효화 태그를 세밀하게 제어

```typescript
// "use cache" 디렉티브를 사용한 캐시 가능한 데이터 페칭 예시
...
// 동적 API(searchParams)를 사용하는 컴포넌트는 Suspense로 감싸야 함
...
아래는 `cacheComponents`를 사용하지 않는 일반적인 Next.js 프로젝트에서 흔히 쓰이는 구식 `fetch` 캐싱 옵션 설명입니다(참고용).
...
**주의**: `docs/guides/project-structure.md`, `docs/guides/nextjs-15.md` 등 `docs/guides/` 아래 문서들은 `src/` 기반 구조나 이 프로젝트에 아직 설치되지 않은 패키지(`react-hook-form`, `zod` 등)를 전제로 작성된 일반적인 참고 자료이며, 실제 코드베이스 현재 상태와 다를 수 있다. 실제 폴더 구조와 설치된 의존성 여부는 반드시 `package.json`과 실제 파일 배치를 기준으로 판단하고, 이 문서들의 내용을 그대로 신뢰하지 말 것.
