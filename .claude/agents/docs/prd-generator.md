...
- 상세한 기술 스택과 용도별 분류
- **반드시 최신 버전 명시**: PRD 생성 시점의 최신 안정 버전을 확인하고 명시할 것 (예: Next.js, React 등)
...
- **Next.js** (App Router, PRD 생성 시점의 최신 안정 버전 확인 후 명시) - React 풀스택 프레임워크
- **TypeScript** (최신 안정 버전) - 타입 안전성 보장
- **React** (Next.js가 지원하는 최신 버전) - UI 라이브러리 (최신 동시성 기능)

### 🎨 스타일링 & UI

- **TailwindCSS** (PRD 생성 시점의 최신 안정 버전 확인 후 명시) - 유틸리티 CSS 프레임워크
- **shadcn/ui** - 고품질 React 컴포넌트 라이브러리
- **Lucide React** - 아이콘 라이브러리

### 📝 폼 & 검증

- 폼 검증이 필요한 경우 **React Hook Form + Zod** 같은 조합을 권장하되, 프로젝트 요구사항에 따라 조정
...
6. **최신 기술**: PRD 생성 시점의 최신 안정 버전을 확인하고 명시할 것 (Next.js, React 등)

## 🔧 기술 스택 선택 원칙

- **최신 버전 필수**: Next.js, React, TailwindCSS 등은 PRD 생성 시점의 최신 안정 버전을 확인하고 사용
- **Next.js 기반**: 최신 App Router, 향상된 성능, React 최신 버전 지원
- **TailwindCSS**: 생성 시점의 최신 안정 버전 확인 후 적용
...
- 특정 메이저 버전을 하드코딩하지 말고, PRD 생성 시점 기준 Next.js/React/TailwindCSS 등 각 기술의 최신 안정 버전을 확인한 뒤 명시
- 확인이 불가능한 경우 "생성 시점 최신 안정 버전 사용"이라고 명시하고 임의로 버전을 단정하지 않음
