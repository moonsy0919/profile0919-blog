...
- 중요 브랜치 보호 (main) — 이 저장소는 `main` 브랜치만 존재하며 `develop` 브랜치는 없음
- 병합 권한 확인
- 코드 리뷰 상태 점검
...
### GitHub/PR 통합 (원격 저장소와 `gh` CLI가 설정되어 있는 경우에만 해당)

> 이 저장소는 현재 원격 저장소가 연결되어 있지 않고 `gh` CLI도 설치되어 있지 않아 아래 기능을 사용할 수 없다.
...
2. **병합 후 검증 실패**
   - 이 프로젝트의 검증 커맨드인 `npm run check-all`(typecheck+lint+format:check)과 `npm run preflight`(check-all+build)를 우선 실행
   - 실패 시 롤백 옵션
   - 수정 후 재시도
