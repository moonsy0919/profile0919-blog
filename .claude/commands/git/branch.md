...
3. 브랜치명 검증 및 규칙 적용
4. 최신 main 브랜치에서 분기 (이 저장소는 `main` 브랜치만 존재하며 `develop` 브랜치는 없음)
5. 새 브랜치 생성 및 전환
...
2. 필요시 변경사항 stash
3. 대상 브랜치로 전환
4. 원격 추적 브랜치 설정 (원격 저장소가 설정되어 있는 경우에만 해당 — 이 저장소는 현재 원격 저장소가 연결되어 있지 않음)
...
- 원격 브랜치와의 동기화 상태 확인 (원격 저장소가 설정되어 있는 경우에만 해당)
- 병합되지 않은 브랜치 삭제 시 경고
- 현재 브랜치에서의 미완료 작업 감지
...
### GitHub CLI 통합 (원격 저장소와 `gh` CLI가 설정되어 있는 경우에만 해당)

> 이 저장소는 현재 원격 저장소(remote)가 연결되어 있지 않고 `gh` CLI도 설치되어 있지 않아 아래 기능을 사용할 수 없다. 원격 저장소와 `gh` CLI를 설정한 이후에만 유효하다.
...
- `--from-issue` : GitHub Issue에서 브랜치 생성 (원격 저장소와 `gh` CLI 필요)
- `--track` : 원격 브랜치 추적 설정 (원격 저장소 필요)
