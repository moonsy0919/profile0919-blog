...
> **사전 조건**: 이 커맨드를 사용하려면 원격 저장소(GitHub)와 `gh` CLI 설정이 필요합니다. `allowed-tools`에 명시된 `gh pr:*`/`gh api:*`/`gh repo:*`와 `git push`는 원격 저장소가 연결되어 있고 `gh auth login`이 완료된 상태에서만 동작합니다. 이 저장소는 현재 `git remote`가 비어 있고 `gh` CLI도 설치되어 있지 않아, 원격 저장소와 `gh` CLI를 먼저 설정하지 않으면 이 커맨드는 사용할 수 없습니다.
...
2. **리뷰어 할당**
   - CODEOWNERS 파일 기반 자동 할당 (CODEOWNERS 파일이 있는 경우에만 해당 — 이 저장소에는 CODEOWNERS 파일이 없음)
   - 팀 구성원 라운드로빈 할당
   - 파일별 전문가 할당
...
- [ ] 기존 테스트 모두 통과 (테스트 스크립트가 정의된 경우에만 해당 — 이 프로젝트는 `package.json`에 `test` 스크립트가 정의되어 있지 않으며, 대신 `npm run check-all`/`npm run preflight` 통과로 대체)
- [ ] 새로운 기능에 대한 테스트 추가 (테스트 스크립트가 정의된 경우에만 해당)
- [ ] 엣지 케이스 테스트 포함 (테스트 스크립트가 정의된 경우에만 해당)
...
## GitHub Actions 통합 (`.github/` 디렉터리에 워크플로우가 설정되어 있는 경우에만 해당)

> 이 저장소는 현재 `.github/` 디렉터리가 없어 GitHub Actions가 구성되어 있지 않다. 아래 기능은 워크플로우를 별도로 설정한 이후에만 유효하다.
